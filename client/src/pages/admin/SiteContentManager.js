  ...
  useEffect(() => {
    if (data) {
      setForm(prev => {
        ...
        // Convert any existing DD/MM/YYYY back to YYYY-MM-DD for the date picker
        const convertToYYYYMMDD = (dStr) => {
          if (!dStr) return "";
        ...
        return {
          ...prev,
          ...data,
          submissionDeadline: convertToYYYYMMDD(data.submissionDeadline || "2025-03-31"),
          publication: convertToYYYYMMDD(data.publication || "2025-06-30"),
          importantDates: incomingDates.map(d => ({ 
            ...d, 
            date: convertToYYYYMMDD(d.date),
